...
export default async function MainLayout({
  ...
  return (
    <div className="layout-wrapper flex h-screen bg-background overflow-hidden">
      {/* Premium Sidebar */}
      <aside className="sidebar-container w-64 border-r border-border/40 bg-card flex flex-col h-full shrink-0">
        <div className="sidebar-inner flex flex-col h-full">
            ...
            <div className="sidebar-section px-4 py-4 border-t border-border/40">
              <div className="upgrade-card bg-primary/5 border border-primary/20 rounded-xl p-4 space-y-3 shadow-sm hover:bg-primary/[0.08] transition-colors duration-200">
                ...
                <p className="upgrade-card-text text-xs text-muted-foreground leading-relaxed">
                  Activate autonomous AI agents to automate your executive workflow.
                </p>
                <Link href="/#pricing" className="block">
                  <Button variant="default" size="sm" className="w-full text-xs font-medium gap-1">
          ...
          {/* User Profile Footer */}
          <div className="sidebar-section p-4 border-t border-border/40 bg-muted/10">
            <div className="user-profile flex items-center justify-between gap-3 bg-muted/40 p-2.5 rounded-xl border border-border/60">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="flex flex-col min-w-0 text-left">
                  <span className="text-xs font-semibold text-foreground truncate">{name || "User"}</span>
                  <span className="text-[10px] text-muted-foreground truncate">{email}</span>
                </div>
              </div>
              {isPaidUser && <Badge variant="secondary" className="text-[9px] bg-primary/10 text-primary hover:bg-primary/20 border-none font-bold uppercase py-0 px-1.5 shrink-0">Pro</Badge>}
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="main-content flex-1 overflow-y-auto bg-background/50">
