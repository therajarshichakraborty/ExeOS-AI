...
const priorityBadges: Record<string, string> = {
  ...
  medium: "bg-yellow-500/10 text-yellow-600 dark:text-yellow-500 hover:bg-yellow-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
...
const categoryBadges: Record<string, string> = {
  ...
  personal: "bg-sky-500/10 text-sky-500 hover:bg-sky-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
  newsletter: "bg-pink-500/10 text-pink-500 hover:bg-pink-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
  notification: "bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
  spam: "bg-red-500/10 text-red-500 hover:bg-red-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
  other: "bg-gray-500/10 text-gray-500 hover:bg-gray-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2",
};
...
export function EmailDetail({ email }: { email: ProcessedEmail }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2 flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {email.status === "error" && (
              <Badge variant="destructive" className="bg-red-500/10 text-red-500 hover:bg-red-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2">
                Failed
              </Badge>
            )}
            {email.priority && (
              <Badge variant="secondary" className={priorityBadges[email.priority] || "bg-muted"}>
                {email.priority}
              </Badge>
            )}
            {email.category && (
              <Badge variant="outline" className={categoryBadges[email.category] || "bg-muted text-muted-foreground"}>
                {email.category}
              </Badge>
            )}
            {email.draftCreated && (
              <Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2 gap-1">
                <FileText className="h-3 w-3" />
                Draft
              </Badge>
            )}
            {(email.tasksCreated ?? 0) > 0 && (
              <Badge variant="outline" className="bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 border-none font-bold text-[10px] uppercase py-0.5 px-2 gap-1">
      ...
      {expanded && (
        <div className="border-t border-border/40 pt-5 space-y-5 animate-fade-in" onClick={(e) => e.stopPropagation()}>
          ...
          {email.status === "error" && (
            <div className="flex items-start gap-3 bg-red-500/5 border border-red-500/20 rounded-xl p-4 text-red-500 text-xs leading-relaxed">
              ...
              <div className="space-y-1">
                ...
                <p>{email.error || "An unknown error occurred during AI analysis."}</p>
              </div>
            </div>
          )}
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-3">
                  {email.actionItems.map((item, i) => (
                    <li key={i} className="border border-border/60 rounded-xl p-4 bg-muted/20 space-y-1.5">
            ...
            {email.draftReply && (
              <div className="space-y-3">
                ...
                <div className="border border-primary/20 rounded-xl p-4 bg-primary/5 space-y-1.5">
                  <p className="text-xs text-muted-foreground font-semibold">Suggested Template</p>
                  <p className="text-xs text-foreground whitespace-pre-wrap leading-relaxed font-sans break-words">{email.draftReply}</p>
                </div>
              </div>
            )}
