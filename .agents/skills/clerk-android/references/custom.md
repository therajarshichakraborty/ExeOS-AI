...
Implement native Android auth with Clerk API primitives while preserving Clerk's
multi-step auth semantics and dashboard-driven capability gating.
...
- Do not add `clerk-android-ui` unless developer explicitly asks for a hybrid
  prebuilt/custom approach.
...
- Verify required project setup (Native API, min SDK/Java, manifest internet
  permission, app-level initialization).
...
- Use Clerk runtime capability/settings fields to drive flow branches (first
  factors, social providers, MFA, Google One Tap support).
...
- Keep branching aligned with factor requirements and verification states
  returned by Clerk.

6. API usage patterns

- Use Clerk sign-in/sign-up APIs and verification methods with structured
...
- For Google, honor runtime One Tap capability and use the appropriate Clerk
  path.
- Do not bypass Clerk by implementing provider-specific token exchange directly
...
- Do not introduce local config indirection for publishable key unless
  requested.
...
- UI, orchestration, and Clerk integration are separated into focused
  files/modules.
