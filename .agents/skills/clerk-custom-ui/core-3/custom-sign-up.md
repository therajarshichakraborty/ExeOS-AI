...
Use `update()` to add optional fields (name, metadata, legal acceptance, locale)
to an existing sign-up before finalization.
...
After successful sign-up and verification, call `finalize()` to activate the
session:
...
If `signUp.isTransferable` is `true`, the identifier matches an existing user
and the sign-up should be transferred to a sign-in flow. This involves
coordinating between sign-up and sign-in resources. See the
[transferable sign-up docs](https://clerk.com/docs/custom-flows/overview) for
...
All methods return `Promise<{ error: ClerkError | null }>`. Errors are also
available reactively on the hook:
...
From
[the docs](https://clerk.com/docs/guides/development/custom-flows/authentication/email-sms-otp).
Uses phone OTP with inline comments for adapting to email OTP.
