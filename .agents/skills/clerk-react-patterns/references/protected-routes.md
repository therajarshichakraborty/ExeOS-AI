...
`ClerkProvider` must be an ancestor of both the router and any Clerk
hooks/components.
...
The `ProtectedRoute` pattern is preferred for route-level control; `<Show>` is
better for component-level.
