...
description:
  "React SPA auth patterns with @clerk/react for Vite/CRA - ClerkProvider setup,
  useAuth/useUser/useClerk hooks, React Router protected routes, custom sign-in
  flows. Triggers on: Vite Clerk setup, React Router auth, useAuth hook,
  protected route, custom sign-in form React."
license: MIT
...
> This skill covers `@clerk/react` for Vite/CRA SPAs. For Next.js use
> `clerk-nextjs-patterns`. For TanStack Start use `clerk-tanstack-patterns`.
...
`@clerk/react` is client-only — there is no server-side `auth()`. All auth state
comes from hooks.

- `isLoaded` must be `true` before trusting `isSignedIn` — always guard on
  `isLoaded`
- `useClerk()` gives access to `signOut`, `openSignIn`, `openUserProfile` and
  other methods
