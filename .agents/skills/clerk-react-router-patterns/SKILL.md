...
description:
  ...
  clerkMiddleware, protected routes, SSR user data, org switching. Triggers on:
  react-router auth, rootAuthLoader, getAuth loader, react-router protected
  route, loader authentication, SSR auth react-router."
license: MIT
...
React Router v7 uses a middleware + loader pipeline. Clerk plugs into both
layers:

- **Middleware** (`clerkMiddleware()`) — runs on every request, attaches auth to
  context
- **`rootAuthLoader`** — required in `root.tsx` to pass Clerk state to the
  client
...
> **Required**: `rootAuthLoader` must be called in `root.tsx`'s loader. Without
> it, `getAuth` throws in nested loaders.
