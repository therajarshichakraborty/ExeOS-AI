...
description:
  "Expo / React Native patterns with Clerk — SecureStore token cache, OAuth deep
  linking, useAuth in native, Expo Router protected routes, push notifications
  with user context. Triggers on: expo clerk, clerk react native, SecureStore
  token cache, expo router auth, OAuth deep link clerk, mobile auth clerk."
license: MIT
...
- **SecureStore** — encrypt token in device keychain (recommended for
  production)
...
> **CRITICAL**: Use `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` — not `NEXT_PUBLIC_`.
> Env vars inside `node_modules` are not inlined in production builds. Always
...
This uses `expo-secure-store` with `keychainAccessible: AFTER_FIRST_UNLOCK`.
