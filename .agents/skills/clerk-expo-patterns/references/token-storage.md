...
Clerk stores the active session token in memory by default. In native apps, use
a persistent token cache so sessions survive app restarts.
...
- `AFTER_FIRST_UNLOCK` means the token is inaccessible until the user unlocks
  the device at least once after a restart
- Always delete the item on `getToken` error — corrupted keychain entries cause
