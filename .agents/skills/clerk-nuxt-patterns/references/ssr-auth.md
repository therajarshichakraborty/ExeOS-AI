...
`@clerk/nuxt` composables (`useAuth`, `useUser`) are client-reactive but the
initial state is SSR-safe.
...
The middleware runs server-side, so by the time the page renders,
unauthenticated users are already redirected.
...
When using middleware, the server redirect happens before hydration, so this is
rarely needed for protected pages.
...
When using Pinia with Clerk, hydrate the store with server auth state to avoid
client-side flash:
...
Stores are created per-request on the server — do NOT store sensitive data
(tokens) in Pinia.
